...
public interface LoginLogMapper extends SuperMapper<LoginLog> {
     ...
     * @return Long
     */
    @Select("select count(1) from c_common_login_log")
     ...
     * @return Long
     */
    @Select("select count(1) from c_common_login_log where login_date = #{today}")
     ...
     * @return Long
     */
    @Select("select count(distinct(request_ip)) from c_common_login_log where login_date = #{today}")
     ...
     * @return
     */
    @Select("select browser, count(id) `count` from c_common_login_log group by browser")
     ...
     */
    @Select("select operating_system,count(id) `count` from c_common_login_log group by operating_system")
