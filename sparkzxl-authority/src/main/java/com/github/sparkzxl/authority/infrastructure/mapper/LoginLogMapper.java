...
import com.github.sparkzxl.authority.infrastructure.entity.LoginLog;
import com.github.sparkzxl.authority.infrastructure.entity.LoginLogCount;
...
public interface LoginLogMapper extends SuperMapper<LoginLog> {
     ...
     * @return Long
     */
    @Select("select count(1) from u_common_login_log")
     ...
     * @return Long
     */
    @Select("select count(1) from u_common_login_log where login_date = #{today}")
     ...
     * @return Long
     */
    @Select("select count(distinct(request_ip)) from u_common_login_log where login_date = #{today}")
    ...
    @Select("<script> "
            ...
            + "        ( select id, login_date from u_common_login_log where login_date >= #{tenDays}\n"
     ...
     * @return
     */
    @Select("select browser, count(id) `count` from u_common_login_log group by browser")
     ...
     */
    @Select("select operating_system,count(id) `count` from u_common_login_log group by operating_system")
     ...
     */
    @Delete("<script> "
            + "delete from u_common_login_log"
            ...
            + "                AND id NOT in(\n"
            + "                SELECT id FROM(\n"
            + "                SELECT id FROM u_common_login_log AS t\n"
