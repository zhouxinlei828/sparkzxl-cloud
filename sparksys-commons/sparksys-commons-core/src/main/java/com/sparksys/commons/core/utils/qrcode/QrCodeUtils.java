...
public class QrCodeUtils {
    ...
    private static BitMatrix createQrCodeMatrix(String content, int length) {
        ...
        try {
            ...
            log.error("内容为：【" + content + "】的二维码生成失败！", e);
            return null;
        }
    ...
    public static byte[] createQrCode(String content, int length, File logoFile) {
        ...
        try {
            ...
            if (logoFile != null) {
                ...
                overlapImage(img, file.getAbsolutePath(), logoFile, new MatrixToLogoImageConfig());
            }
     ...
     */
    private static void overlapImage(BufferedImage image, String imagePath, File logoFile,
        ...
        try {
            ...
            g.dispose();
            // 写入logo图片到二维码
            ImageIO.write(image, QrCodeUtils.FORMAT, new File(imagePath));
        } catch (Exception e) {
