...
public interface LoginLogMapper extends SuperMapper<LoginLog> {
     ...
     * @return Long
     */
    @Select("select count(1) from common_login_log")
     ...
     * @return Long
     */
    @Select("select count(1) from common_login_log where login_date = #{today}")
     ...
     * @return Long
     */
    @Select("select count(distinct(request_ip)) from common_login_log where login_date = #{today}")
    ...
    @Select("<script> "
            ...
            + "        ( select id, login_date from common_login_log where login_date >= #{tenDays}\n"
     ...
     * @return
     */
    @Select("select browser, count(id) `count` from common_login_log group by browser")
     ...
     */
    @Select("select operating_system,count(id) `count` from common_login_log group by operating_system")
    ...
    @Delete("<script> "
            ...
            + "                AND id NOT in(\n"
            + "                SELECT id FROM(\n"
            + "                SELECT id FROM common_login_log AS t\n"
